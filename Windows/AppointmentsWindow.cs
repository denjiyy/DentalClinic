...
namespace DentalClinicManagement
{
    public partial class AppointmentsWindow : Form
    {
        ...
        {
            PrescriptionWindow prescription = new();
            prescription.Show();
            this.Hide();
        }
        ...
        private void SaveToXML_Click(object sender, EventArgs e)
        {
            ...
            {
                // get the selected dentist from the combobox
                Dentist selectedDentist = (Dentist)dentistCb.SelectedItem;

                // filter the list of dentists to include only the selected dentist
                List<Dentist> filteredDentists = dentists.Where(d => d.Name == selectedDentist.Name).ToList();

                // create an instance of Save and populate its Appointments and Dentists properties
                Save recordsToSave = new Save();
                recordsToSave.Appointments = records;
                recordsToSave.Dentists = filteredDentists;

                // serialize the Save instance and save it to the file
        ...
        private void dentistCb_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (dentistCb.SelectedItem != null)
            {
                ...
                // display the selected dentist's information in the DataGridView
                if (DentistDGV.Columns.Count == 0)
                {
                    DentistDGV.Columns.Add("Name", "Name");
                    DentistDGV.Columns.Add("Specialty", "Specialization");
                    DentistDGV.Columns.Add("Age", "Age");
                    DentistDGV.Columns.Add("Gender", "Gender");
                }
                DentistDGV.Rows.Clear();
                DentistDGV.Rows.Add(selectedDentist.Name, selectedDentist.Specialty, selectedDentist.Age, selectedDentist.Gender);
            }
        }
    }
