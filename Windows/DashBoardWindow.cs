...
namespace DentalClinicManagement
{
    public partial class DashBoardWindow : Form
    {
        ...
        private void DashBoardWindow_Load(object sender, EventArgs e)
        {
            ...
            NextAppointmentCircle.Value = 100;
            PatientCircle.Value = 100;
            SqlConnection connection = conn.GetConnection();
            connection.Open();
            SqlDataAdapter adapter = new SqlDataAdapter("select count(*) from AppointmentTable", connection);
            ...
            SqlDataAdapter adapter3 = new SqlDataAdapter("select count(*) from UserTable", connection);
            DataTable dataTable3 = new DataTable();
            adapter3.Fill(dataTable3);
            User.Text = dataTable3.Rows[0][0].ToString();
            SqlDataAdapter adapter4 = new SqlDataAdapter("select min(AppointmentDate) from AppointmentTable", connection);
            DataTable dataTable4 = new DataTable();
            adapter4.Fill(dataTable4);
            NextAppointment.Text = dataTable4.Rows[0][0].ToString();
            connection.Close();
        }
